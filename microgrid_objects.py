...
class GridActor:
    ...
    def add_source(self, source, val: bool=True):
        ...
        self.receives_from[str(source)] = Node(f'{str(self)} receives from {str(source)}', 
    ...
    def add_active_source(self, source, val: bool=None):
        ...
        self.receiving_from[str(source)] = Node(f'{str(self)} receiving from {str(source)}', 
            val, description=description)
...
class Generator(GridActor):
    ...
    def __init__(self, name: str, fuel_capacity: float, starting_fuel_level: float, 
                 max_output: float, efficiency: float, **kwargs):
        self.fuel_capacity = Node(f'fuel_capacity_{name}', fuel_capacity, 
            description='max amount of fuel in generator', units='Gal')
        self.starting_fuel_level = Node(f'starting_fuel_level_{name}', starting_fuel_level, 
            description='starting fuel level in generator', units='Gal')
        self.max_output = Node(f'max_output_{name}', max_output, 
            description='max charge generator can output', units='W')
        self.efficiency = Node(f'efficiency_{name}', efficiency, 
            description='efficiency for generator', units='Gph/W')
        self.fuel_level = Node(f'fuel_level_{name}', kwargs.get('fuel_level', None), 
            description='amount of fuel in generator', units='Gal')
        self.out_of_fuel = Node(f'out_of_fuel_{name}', kwargs.get('out_of_fuel', None), 
...
class Battery(GridActor):
    ...
    def __init__(self, name: str, charge_level: float, charge_capacity: float, max_output: float, 
        ...
        self.charge_level = Node(f'charge_level_{name}', charge_level, 
            description='amount of charge in battery')
        self.charge_capacity = Node(f'charge_capacity_{name}', charge_capacity, 
            description='max amount of charge in battery')
        self.wasted_charge = Node(f'wasted_charge_{name}', 0., 
            description='charge wasted in battery')
        self.max_output = Node(f'max_output_{name}', max_output, 
            description='max power battery can output', units='W')
        self.efficiency = Node(f'efficiency_{name}', efficiency, 
            description='efficiency of battery')
        self.max_charge_rate = Node(f'max_charge_rate_{name}', max_charge_rate, 
            description='max charge rate for battery', units='W/hr')
        self.is_charging = Node(f'{name} is charging', 
...
class Building(GridActor):
    ...
    def __init__(self, name: str, type: BUILDING_TYPE, **kwargs):
        ...
        self.equipment_load = Node(f'equipment_load_{name}', 
            kwargs.get('equipment_load', None),
            description='equipment_load of building')
        self.building_filename = Node(f'building_filename_{name}', 
        ...
        self.equipment_col_name = Node(f'equipment_col_name_{name}', 
